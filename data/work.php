<?php 

// Note: key must be identical to slug value, matching the corrisponding detail page in the work folder and folder in assets/images/work/
$work = [
	"doordash" => [
		"slug" => "doordash",
		"company" => "DoorDash",
		"brief" => "DoorDash Launch",
		"thumb" => "DoorDash-thumb-420x600.webp",
		"thumb_sq" => "DoorDash-thumb-650x650.webp",
		"thumb_primary_m" => "DoorDash-mobile-1167x544.webp",
		"gradient" => false,
		"tags" => [
			"Brand",
			"OOH",
			"TVC",
			"Social",
		],
	],
	"co-operative-bank" => [
		"slug" => "co-operative-bank",
		"company" => "The Co-Operative Bank",
		"brief" => "#BankBetterNZ",
		"thumb" => "Coop_Bank-thumb-420x600.webp",
		"thumb_sq" => "Coop_Bank-thumb-650x650.webp",
		"thumb_primary_m" => "CoopBank-mobile-1167x544.webp",
		"gradient" => true,
		"tags" => [
			"TVC",
			"OOH",
			"Digital",
		],
	],
	"kiwirail" => [
		"slug" => "kiwirail",
		"company" => "KiwiRail",
		"brief" => "Steely Stare",
		"thumb" => "Kiwirail-thumb-420x600.webp",
		"thumb_sq" => "Kiwirail-thumb-650x650.webp",
		"thumb_primary_m" => "KiwiRail-mobile-1167x544.webp",
		"gradient" => true,
		"tags" => [
			"Social",
			"Digital",
			"OOH",
			"PR",
		],
	],
	"maritime" => [
		"slug" => "maritime",
		"company" => "Maritime NZ",
		"brief" => "Kia Mataara",
		"thumb" => "thumb-420x600.webp",
		"thumb_sq" => "thumb-650x650.webp",
		"thumb_primary_m" => "Kia-Mataara-mobile-1167x544.webp",
		"gradient" => true,
		"tags" => [
			"Brand",
			"OOH",
			"TVC",
			"Social",
			"Website"
		],
	],
  "nzo-macbeth" => [
		"slug" => "nzo-macbeth",
		"company" => "NZ Opera",
		"brief" => "The Paranoid Campaign",
		"thumb" => "thumb-420x600.webp",
		"thumb_sq" => "thumb-650x650.webp",
		"thumb_primary_m" => "Macbeth-mobile-1167x544.webp",
		"gradient" => true,
		"tags" => [
			"OOH",
			"Digital",
			"Radio",
		],
	],
	"massey" => [
		"slug" => "massey",
		"company" => "Massey University",
		"brief" => "Success From a Thousand Little Moments",
		"thumb" => "thumb-420x600.webp",
		"thumb_sq" => "thumb-650x650.webp",
		"thumb_primary_m" => "Massey-mobile-1167x544.webp",
		"gradient" => false,
		"tags" => [
			"PR",
			"OOH",
			"Digital",
			"Social",
		],
	],
	"at-mission-electric" => [
		"slug" => "at-mission-electric",
		"company" => "Auckland Transport",
		"brief" => "Mission Electric",
		"thumb" => "thumb-420x600.webp",
		"thumb_sq" => "thumb-650x650.webp",
		"thumb_primary_m" => "thumb-650x650.webp",
		"gradient" => true,
		"tags" => [
			"OOH",
		],
	],
	"at-proof" => [
		"slug" => "at-proof",
		"company" => "Auckland Transport",
		"brief" => "Let's go there",
		"thumb" => "thumb-420x600.webp",
		"thumb_sq" => "thumb-650x650.webp",
		"thumb_primary_m" => "thumb-650x650.webp",
		"gradient" => true,
		"tags" => [
			"TVC",
			"OOH",
			"Social",
			"Digital",
			"Radio",
		],
	],
	"nzo-unruly-tourists" => [
		"slug" => "nzo-unruly-tourists",
		"company" => "NZ Opera",
		"brief" => "Unruly tourists",
		"thumb" => "Unruly-Tourists-thumb-420x600.webp",
		"thumb_sq" => "Unruly-Tourists-thumb-650x650.webp",
		"thumb_primary_m" => "Unruly-Tourists-thumb-650x650.webp",
		"gradient" => true,
		"tags" => [
			"Brand",
			"OOH",
			"Social",
		],
	],
	"auckland-airport-looking-beyond" => [
		"slug" => "auckland-airport-looking-beyond",
		"company" => "Auckland Airport",
		"brief" => "Looking Beyond",
		"thumb" => "thumb-420x600.webp",
		"thumb_sq" => "thumb-650x650.webp",
		"thumb_primary_m" => "Auckland-Airport-mobile-1167x544.webp",
		"gradient" => false,
		"tags" => [
			"Brand Design",
		],
	],
	"eke-panuku" => [
		"slug" => "eke-panuku",
		"company" => "Eke Panuku",
		"brief" => "Rebrand",
		"thumb" => "thumb-420x600.webp",
		"thumb_sq" => "thumb-650x650.webp",
		"thumb_primary_m" => "EkePanuku-mobile-1167x544.webp",
		"gradient" => true,
		"tags" => [
			"Brand",
			"Website",
			"UX",
		],
	],
	"blues" => [
		"slug" => "blues",
		"company" => "Blues",
		"brief" => "Paint it Blue",
		"thumb" => "thumb-420x600.webp",
		"thumb_sq" => "thumb-420x600.webp",
		"thumb_primary_m" => "Blues-mobile-1167x544.webp",
		"gradient" => true,
		"tags" => [
			"Brand",
			"OOH",
			"Social",
		],
	],
	"auckland-unlimited-sounds" => [
		"slug" => "auckland-unlimited-sounds",
		"company" => "Tātaki Auckland Unlimited",
		"brief" => "Hear. See. Feel.",
		"thumb" => "Sound_of_Auckland-thumb-420x600.webp",
		"thumb_sq" => "Sound_of_Auckland-thumb-650x650.webp",
		"thumb_primary_m" => "",
		"gradient" => true,
		"tags" => [
			"Social",
			"Audio",
		],
	],
	"sue-the-sea" => [
		"slug" => "sue-the-sea",
		"company" => "Sustainable Business Network",
		"brief" => "Sue the Sea",
		"thumb" => "Sue_the_Sea-thumb-420x600.webp",
		"thumb_sq" => "Sue_the_Sea-thumb-650x650.webp",
		"thumb_primary_m" => "",
		"gradient" => true,
		"tags" => [
			"Content",
			"Digital",
		],
	],
	"auckland-unlimited" => [
		"slug" => "auckland-unlimited",
		"company" => "Tātaki Auckland Unlimited",
		"brief" => "Iconic Eats",
		"thumb" => "thumb-420x600.webp",
		"thumb_sq" => "thumb-650x650.webp",
		"thumb_primary_m" => "",
		"gradient" => true,
		"tags" => [
			"OOH",
			"Social",
			"Digital",
		],
	],
	"at-bike-helmets" => [
		"slug" => "at-bike-helmets",
		"company" => "Auckland Transport",
		"brief" => "Biking Makes",
		"thumb" => "thumb-420x600.webp",
		"thumb_sq" => "thumb-650x650.webp",
		"thumb_primary_m" => "",
		"gradient" => true,
		"tags" => [
			"OOH",
		],
	],
	"emirates" => [
		"slug" => "emirates",
		"company" => "Emirates",
		"brief" => "Door to Bali",
		"thumb" => "thumb-420x600.webp",
		"thumb_sq" => "thumb-650x650.webp",
		"thumb_primary_m" => "",
		"gradient" => true,
		"tags" => [
			"Activation",
			"PR",
			"Ambient",
			"Experiential",
		],
	],
	"at-drive-karma" => [
		"slug" => "at-drive-karma",
		"company" => "Auckland Transport",
		"brief" => "Drive Karma",
		"thumb" => "thumb-420x600.webp",
		"thumb_sq" => "thumb-650x650.webp",
		"thumb_primary_m" => "",
		"gradient" => true,
		"tags" => [
			"TVC",
			"OOH",
			"Social",
			"Brand",
		],
	],
	"auckland-council" => [
		"slug" => "auckland-council",
		"company" => "Auckland Council Whānau",
		"brief" => "Oh So Many",
		"thumb" => "thumb-420x600.webp",
		"thumb_sq" => "thumb-650x650.webp",
		"thumb_primary_m" => "",
		"gradient" => true,
		"tags" => [
			"OOH",
			"Social",
			"Digital",
		],
	],
	"at-see-everything" => [
		"slug" => "at-see-everything",
		"company" => "Auckland Transport",
		"brief" => "See Everything",
		"thumb" => "thumb-420x600.webp",
		"thumb_sq" => "thumb-650x650.webp",
		"thumb_primary_m" => "",
		"gradient" => true,
		"tags" => [
			"OOH",
			"Digital",
		],
	],
  "true-honey" => [
		"slug" => "true-honey",
		"company" => "True Honey Co.",
		"brief" => "True Honey e&#8209;commerce",
		"thumb" => "thumb-420x600.webp",
		"thumb_sq" => "thumb-650x650.webp",
		"thumb_primary_m" => "TrueHoneyCo-mobile-1167x544.webp",
		"gradient" => true,
		"tags" => [
			"Brand",
			"Website",
			"Shopify",
			"UI",
			"UX",
			"CX",
		],
	],
];